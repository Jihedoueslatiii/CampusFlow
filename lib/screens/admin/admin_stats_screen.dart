...
class _AdminStatsScreenState extends State<AdminStatsScreen> with SingleTickerProviderStateMixin {
  ...
  // 3. WIDGET AMÉLIORÉ : Carte de Statistique (inchangé)
  ...
  Widget _buildRolePieChart(Map<String, int> distribution, int total) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Répartition des Rôles", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  // 5. WIDGET GRAPHE À BARRES (Bar Chart Simulé - inchangé)
  Widget _buildMonthlyBarChart(Map<String, int> monthlySignups) {
    if (monthlySignups.isEmpty) {
      return const Center(child: Text("Aucune donnée d'inscription récente."));
    }
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Inscriptions Mensuelles", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const Divider(thickness: 1, color: Colors.grey),
            ...
            child: Row(
              ...
              children: monthlySignups.entries.map((entry) {
                ...
                return Column(
                  ...
                  children: [
                    Text(entry.value.toString(), style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 4),
                    Container(
                    ...
                    const SizedBox(height: 4),
                    Text(entry.key, style: TextStyle(fontSize: 14, color: Colors.grey.shade700)),
                  ],
                );
              }).toList(),
            ),
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data == null) {
            return const Center(child: Text('Aucune donnée de statistique trouvée.'));
          }
