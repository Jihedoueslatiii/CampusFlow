        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('Participation annulée'),
                  backgroundColor: primaryAccent,
                ),
              );
              _loadData();
...
class EventCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      elevation: 2,
      color: _EventScreenState.background,
      child: ListTile(
        leading: Icon(
          isUpcoming ? Icons.event_available : Icons.event_busy,
          color: isUpcoming ? _EventScreenState.primaryAccent : Colors.grey,
        ),
        title: Text(
          event.title,
          style: TextStyle(
            ...
            fontSize: 16,
            color: isUpcoming ? _EventScreenState.textPrimary : Colors.grey,
          ),
        ),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                fontSize: 14,
                color: _EventScreenState.textPrimary.withOpacity(0.8),
              ),
            ),
            const SizedBox(height: 4),
            Text(
              ...
              style: TextStyle(
                fontSize: 12,
                color: _EventScreenState.primaryAccent,
              ),
            ),
            const SizedBox(height: 4),
            Text(
              ...
              style: TextStyle(
                fontSize: 12,
                color: _EventScreenState.textPrimary.withOpacity(0.6),
              ),
            ),
            const SizedBox(height: 4),
            Text(
              ...
              style: TextStyle(
                ...
                color: _EventScreenState.textPrimary.withOpacity(0.7),
              ),
            ),
            const SizedBox(height: 4),
            Text(
        ...
        trailing: Row(
          ...
          children: [
            if (userRole == 'admin' || userRole == 'professeur') ...[
              IconButton(
                icon: Icon(Icons.edit, color: _EventScreenState.primaryAccent),
                onPressed: onEdit,
                tooltip: 'Modifier',
              ),
              IconButton(
                icon: Icon(Icons.delete, color: _EventScreenState.primaryAccent),
                onPressed: onDelete,
                tooltip: 'Supprimer',
              ),
            ],
    ...
    return FutureBuilder<bool>(
      ...
      builder: (context, snapshot) {
        ...
        return Text(
          ...
          style: TextStyle(
            fontSize: 11,
            color: isParticipating ? Colors.green : _EventScreenState.primaryAccent,
            fontWeight: FontWeight.bold,
          ),
        );
      },
...
class _EventDialogState extends State<EventDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      backgroundColor: _EventScreenState.background,
      title: Text(
        isEditing ? 'Modifier l\'Événement' : 'Nouvel Événement',
        style: TextStyle(color: _EventScreenState.textPrimary),
      ),
      content: Form(
        key: _formKey,
            ...
            children: [
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: 'Titre',
                  labelStyle: TextStyle(color: _EventScreenState.textPrimary.withOpacity(0.7)),
                  border: const OutlineInputBorder(),
                  focusedBorder: OutlineInputBorder(
                    borderSide: BorderSide(color: _EventScreenState.primaryAccent),
                  ),
                ),
                style: TextStyle(color: _EventScreenState.textPrimary),
                validator: (value) {
              ...
              DropdownButtonFormField<int>(
                ...
                dropdownColor: _EventScreenState.background,
                style: TextStyle(color: _EventScreenState.textPrimary),
                  ...
                  Expanded(
                    child: OutlinedButton(
                      style: OutlinedButton.styleFrom(
                        foregroundColor: _EventScreenState.textPrimary,
                        side: BorderSide(color: _EventScreenState.textPrimary.withOpacity(0.3)),
                      ),
      ...
      actions: [
        TextButton(
          ...
          child: Text(
            'Annuler',
            style: TextStyle(color: _EventScreenState.textPrimary.withOpacity(0.7)),
          ),
        ),
        ElevatedButton(
          style: ElevatedButton.styleFrom(
            backgroundColor: _EventScreenState.primaryAccent,
            foregroundColor: _EventScreenState.background,
          ),
